    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> BPM y Ofimática</strong>
              <small> Lista de BPM y Ofimática</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="idBPM"
                :rowHover="true"
                :selection.sync="bpmSeleccionado"
              ...
              >
                ...
                <template #empty> No se encontraron BPM. </template>
                <template #loading>
                  Cargando la información de BPM. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.bpMs = bpMs.map(({ idBPM, nombre, descripcion }) => ({
        idBPM: idBPM,
        nombre: nombre,
