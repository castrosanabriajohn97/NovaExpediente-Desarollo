    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Idioma</strong>
              <small> Lista de idiomas</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="idIdioma"
                :rowHover="true"
                :selection.sync="idiomaSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} idiomas"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-idiomas"
              ...
              >
                ...
                <template #empty> No se encontraron idiomas. </template>
                <template #loading>
                  Cargando la información de idiomas. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.idiomas = idiomas.map(({ idIdioma, nombre, descripcion }) => ({
        idIdioma: idIdioma,
        nombre: nombre,
