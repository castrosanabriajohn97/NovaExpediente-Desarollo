    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Buses y SOA</strong>
              <small> Lista de buses y SOA</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="idBus"
                :rowHover="true"
                :selection.sync="busSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} buses"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-buses"
              ...
              >
                ...
                <template #loading>
                  Cargando la información de buses y SOA. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
<script>
import { GetBusConsulta } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.buses = buses.map(
        ({ idBus, nombre, descripcion }) => ({
          idBus: idBus,
          nombre: nombre,
