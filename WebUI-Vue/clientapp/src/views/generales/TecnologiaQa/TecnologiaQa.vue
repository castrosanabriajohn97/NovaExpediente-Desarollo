    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Tecnología en Calidad</strong>
              <small> Lista de Tecnologías QA</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="idcalidad"
                :rowHover="true"
                :selection.sync="tecnologiaCalidadSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} tecnologías en calidad"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-calidads"
              ...
              >
                ...
                <template #empty> No se encontraron tecnologías en calidad. </template>
                <template #loading>
                  Cargando la información de tecnologías en calidad. Por favor espere.
                </template>
               <Column
                  field="Nombre"
                  header="Nombre"
...
<script>
import { GetCalidad } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.calidads = calidads.map(({ idcalidad, nombre, descripcion }) => ({
        idcalidad: idcalidad,
        nombre: nombre,
