    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Proveedor Certificación</strong>
              <small> Lista de proveedores de certificaciones</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                :value="proveedorCertificaciones"
                :paginator="true"
                class="p-datatable-consultas p-datatable-striped p-datatable-sm"
                :rows="10"
                dataKey="idProveedorCertificacion"
                :rowHover="true"
                :selection.sync="proveedorCertificacionSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} provedores de certificaciones"
                sortField="nombre"
                :sortOrder="1"
                stateStorage="session"
                stateKey="dt-state-proveedorCertificaciones"
              ...
              >
                ...
                <template #empty> No se encontraron proveedores de certificaciones. </template>
                <template #loading>
                  Cargando la información de proveedores de certificaciones. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  name: "proveedorCertificaciones",
  data() {
    return {
      proveedorCertificaciones: [],
      proveedorCertificacionSeleccionado: [],
      loading: true,
      filters: {},
    };
  },
  ...
  methods: {
    async cargarTabla() {
      ...
      this.proveedorCertificaciones = proveedorCertificaciones.map(({ idProveedorCertificacion, nombre, descripcion }) => ({
