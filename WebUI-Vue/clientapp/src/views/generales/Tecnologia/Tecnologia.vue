    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Tecnología</strong>
              <small> Lista de tecnologías</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                :rows="10"
                dataKey="idtecnologia"
                :rowHover="true"
                :selection.sync="tecnologiaSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} tecnologías"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-tecnologias"
              ...
              >
                ...
                <template #empty> No se encontraron tecnologías. </template>
                <template #loading>
                  Cargando la información de tecnologías. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
<script>
import { GetTecnologia } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.tecnologias = tecnologias.map(({ idtecnologia, nombre, descripcion,versiones }) => ({
