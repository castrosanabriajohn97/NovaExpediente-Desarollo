    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> ERP</strong>
              <small> Lista de ERP</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="iderpcore"
                :rowHover="true"
                :selection.sync="erpSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} ERP"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-erpCores"
              ...
              >
                ...
                <template #empty> No se encontraron ERP. </template>
                <template #loading>
                  Cargando la información de ERP. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
                ...
                <Column
                  ...
                  filterMatchMode="tecnologia"
                  headerStyle="width:30%;"
                >
                <template #body="slotProps">
                    <span class="p-column-title">Tecnología</span>
                    {{ slotProps.data.tecnologia }}
                  </template>
                </Column>
...
<script>
import { GetErpCore } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.erpCores = erpCores.map(({ iderpcore, nombre, descripcion,tecnologia }) => ({
        iderpcore: iderpcore,
        nombre: nombre,
