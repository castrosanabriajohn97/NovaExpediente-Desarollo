    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Empresa</strong>
              <small> Lista de empresas</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                :value="empresaCorporacions"
                :paginator="true"
                class="p-datatable-consultas p-datatable-striped p-datatable-sm"
                :rows="10"
                dataKey="idempresacorporacion"
                :rowHover="true"
                :selection.sync="empresaSeleccionado"
              ...
              >
                ...
                <template #empty> No se encontraron empresas. </template>
                <template #loading>
                  Cargando la información de empresas. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
<script>
import { GetEmpresaCorporacion } from "./request";

export default {
  name: "empresaCorporacion",
  data() {
    return {
      empresaCorporacions: [],
      empresaSeleccionado: [],
  ...
  methods: {
    async cargarTabla() {
      ...
      this.empresaCorporacions = empresaCorporacions.map(({ idempresacorporacion, nombre, descripcion }) => ({
