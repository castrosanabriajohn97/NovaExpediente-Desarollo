    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Sector</strong>
              <small> Lista de sectores</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                dataKey="idSector"
                :rowHover="true"
                :selection.sync="sectorSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} sectores"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-sectores"
              ...
              >
                ...
                <template #empty> No se encontraron sectores. </template>
                <template #loading>
                  Cargando la información de sectores. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.sectores = sectores.map(({ idSector, nombre, descripcion }) => ({
        idSector: idSector,
        nombre: nombre,
