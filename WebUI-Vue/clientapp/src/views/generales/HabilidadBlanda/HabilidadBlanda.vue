    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Habilidades Blandas</strong>
              <small> Lista de habilidades blandas</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                :rows="10"
                dataKey="idHabilidadBlanda"
                :rowHover="true"
                :selection.sync="habilidadBlandaSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} habilidades blandas"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-habilidadesBlandas"
              ...
              >
                ...
                <template #empty> No se encontraron habilidades blandas. </template>
                <template #loading>
                  Cargando la información de habilidades blandas. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.habilidadesBlandas = habilidadesBlandas.map(({ idHabilidadBlanda, nombre, descripcion }) => ({
        idHabilidadBlanda: idHabilidadBlanda,
        nombre: nombre,
