    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Lenguajes de Programación</strong>
              <small> Lista de lenguajes de programación</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                :rows="10"
                dataKey="idlenguajeprogramacion"
                :rowHover="true"
                :selection.sync="lenguajeProgramacionSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} lenguajes de programación"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-lenguajesProgramacion"
              ...
              >
                ...
                <template #empty> No se encontraron lenguajes de programacion. </template>
                <template #loading>
                  Cargando la información de lenguajes de programacion. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
                ...
                <Column
                  ...
                  header="Versiones"
                  sortField="versiones"
                  filterField="versiones"
                  filterMatchMode="contains"
                  :sortable="true"
                  
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Versiones</span>
                    {{ slotProps.data.versiones }}
                  </template>
                </Column>
...
<script>
import { GetLenguajeProgramacion } from "./request";
...
export default {
  ...
  data() {
    return {
      ...
      lenguajeProgramacionSeleccionado: [],
      loading: true,
      filters: {},
    };
  },
  ...
  methods: {
    async cargarTabla() {
      ...
      this.lenguajesProgramacion = lenguajesProgramacion.map(({ idlenguajeprogramacion, nombre, descripcion,versiones }) => ({
