    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Certificación</strong>
              <small> Lista de certificaciones</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                :rows="10"
                dataKey="idcertificacion"
                :rowHover="true"
                :selection.sync="certificacionSeleccionado"
              ...
              >
                ...
                <Column
                  field="proveedoresList"
                  header="Proveedores"
                  :sortable="true"
                  sortField="proveedoresList"
                  filterField="proveedoresList"
                  filterMatchMode="contains"
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Proveedores</span>

                    <CBadge
                      v-for="item in slotProps.data.proveedoresList"
                      :key="item"
                      :color="item == 'Sin Definir' ? 'warning' : 'info'"
                      class="mr-2"
                    >
                      {{ item }}
                    </CBadge>
...
<script>
import { GetCertificacionProveedores } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.certificaciones = certificacionesProveedores.map(
        ({ idcertificacion, nombre, descripcion, proveedores }) => ({
          idcertificacion: idcertificacion,
          nombre: nombre,
          descripcion: descripcion,
          proveedores: proveedores,
          proveedoresList: (proveedores == "" ? "Sin Definir" : proveedores).split(",")
        })
      );
    },
