    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Grados Académicos</strong>
              <small> Lista de grados académicos</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                :value="gradosAcademicos"
                :paginator="true"
                class="p-datatable-consultas p-datatable-striped p-datatable-sm"
                :rows="10"
                dataKey="id"
                :rowHover="true"
                :selection.sync="gradoAcademicoSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} grados académicos"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-gradoAcademico"
              ...
              >
                ...
                <template #empty> No se encontraron grados académicos. </template>
                <template #loading>
                  Cargando la información de grados académicos. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.gradosAcademicos = gradosAcademicos.map(({ id, nombre, descripcion,orden}) => ({
        id: id,
        nombre: nombre,
