          ...
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong>
                Centro de estudio</strong
              >
              <small> Lista de centro de estudio</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                :value="centrosEstudio"
                :paginator="true"
                class="p-datatable-consultas p-datatable-striped p-datatable-sm"
                :rows="10"
                dataKey="idCentroEstudio"
                :rowHover="true"
                :selection.sync="centroEstudioSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} centros de estudio"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-centroEstudio"
              ...
              >
                ...
                <template #empty>
                  No se encontraron centros de estudio.
                </template>
                <template #loading>
                  Cargando la información de centros de estudio. Por favor
                  espere.
                </template>
                <Column
                  field="Nombre"
...
<script>
import { GetCentrosEstudio } from "./request";
 ...
 async mounted() {
  ...
  methods: {
   async cargarTabla() {
      ...
      this.centrosEstudio = centrosEstudios.map(({ idCentroEstudio, nombre, descripcion }) => ({
