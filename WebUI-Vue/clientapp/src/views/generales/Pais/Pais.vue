    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Pais</strong>
            ...
            <CCardBody>
              <DataTable
                ...
                dataKey="idpais"
                :rowHover="true"
                :selection.sync="paisSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} paises"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-paises"
              ...
              >
                ...
                <template #empty> No se encontraron paises. </template>
                <template #loading>
                  Cargando la información de paises. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
                ...
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Código Pais</span>
                    {{ slotProps.data.codpais }}
                  </template>
                </Column>
              </DataTable>
            </CCardBody>
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.paises = paises.map(({ idpais, nombre, codpais }) => ({
        idpais: idpais,
        nombre: nombre,
        codpais: codpais,
      }));
