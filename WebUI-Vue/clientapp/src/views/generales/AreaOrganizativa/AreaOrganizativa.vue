    ...
    <div class="wrapper vld-parent">
      <CRow>
        <CCol lg="12">
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong> Área organizativa</strong>
              <small> Lista de Áreas organizativa</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
                ...
                :rows="10"
                dataKey="idareaorganizativa"
                :rowHover="true"
                :selection.sync="areaSeleccionado"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} BPM"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-areas"
              ...
              >
                ...
                <template #empty> No se encontraron áreas organizativas. </template>
                <template #loading>
                  Cargando la información de áreas organizativas. Por favor espere.
                </template>
                <Column
                  field="Nombre"
                  header="Nombre"
...
<script>
import { GetAreasOrganizativas } from "./request";
...
export default {
  ...
  methods: {
    async cargarTabla() {
      ...
      this.areas = areaOrganizativas.map(({ idareaorganizativa, nombre, descripcion }) => ({
        idareaorganizativa: idareaorganizativa,
        nombre: nombre,
