...
<template>
  <div>
    <div class="wrapper vld-parent">
      ...
      >
        <strong>Categorías de Noticias</strong>
        <small>Listado de Categorías de Noticias</small>
      </CCard>
          ...
          <CCard>
            ...
            <CCardBody>
              <DataTable
                ...
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} categorías de noticias"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-categorias-noticias"
                selectionMode="single"
                :autoLayout="true"
              >
                <template #empty> No se encontraron categorías. </template>
                <template #loading>
                  Cargando la información de categorías. Por favor espere.
                </template>
                <Column
                  field="nombre"
                  header="Nombre"
...
import { Categories, SaveCategory, DeleteCategory } from "./request";
...
export default {
  ...
  methods: {
    ...
    async cargarTabla() {
      ...
      this.categories = noticiasCategorias.map(
        ({ nombre, descripcion, idNoticiaCategoria }) => ({
    ...
    createCategory() {
      this.tituloModalEdicion = "Agregar nueva categoría de noticias";
    ...
    async editCategory(item, _x, _event) {
      this.tituloModalEdicion = "Editar categoría de noticias";
      this.tipoModal = "Editar";
      this.isEdit = true;
      this.selectedCategory = item;
      this.newCategory.id = item.idNoticiaCategoria;
    ...
    async deleteCategory(item, _x, _event) {
      const id = item.idNoticiaCategoria;
      const res = await this.Confirmacion_Generico(
        `Desea eliminar la categoría ${item.nombre}, ¡Esto conlleva la eliminación de todas sus noticias relacionadas!`,
        "Eliminar Categoría"
      );
      if (res) {
        await DeleteCategory(id)
          .then(() => {
            Vue.$toast.success(
              "La categoría de noticia fue eliminada exitosamente.",
              {}
            );
          })
          .catch((response) => {
            console.log(response.Error);
            Vue.$toast.error("La categoría de noticia no pudo eliminarse.");
          });
        await this.cargarTabla();
      } else {
        // mostrar error
    ...
    async submitForm() {
      await SaveCategory({
        ...
        .then(() => {
          Vue.$toast.success("La información fue enviada exitosamente.");
        })
        .then(() => {
          setTimeout(async () => {
            await this.cargarTabla();
        ...
        .catch(() => {
          Vue.$toast.error("La información no se pudo enviar.");
        });
