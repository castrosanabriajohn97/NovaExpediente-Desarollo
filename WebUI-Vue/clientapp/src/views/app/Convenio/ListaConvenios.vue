...
<template>
  <div>
    ...
    <DataTable
      ...
      dataKey="idConvenio"
      :rowHover="true"
      :selection.sync="selectedConvenio"
      :filters.sync="filters"
      :loading="loading"
      paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
      :rowsPerPageOptions="[5, 10, 25, 50]"
      currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} convenios"
      sortField="idConvenio"
      :sortOrder="1"
      stateKey="dt-state-convenios"
    ...
    >
      ...
      <template #empty> No se encontraron convenios. </template>
      <template #loading>
        Cargando la información de convenios. Por favor espere.
      </template>
      <Column
        field="nombre"
        header="Nombre"
...
import { GetConvenios, GetConveniosCategorias, Languages, DeleteConvenio } from "../request";
...
export default {
  ...
  async mounted() {
    ...
    await this.getCategoriasConvenios();
  ...
  methods: {
    ...
    view(item, _x, _event) {
      ...
      this.$emit("view", this.selectedConvenio.idConvenio);
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    edit(item, _x, _event) {
      this.selectedConvenio = item;
      this.$emit("edit", this.selectedConvenio.idConvenio);
    },
    create() {
      this.$emit("create");
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    async deleteConvenio(item, _x, _event) {
      const id = item.idConvenio;
      ...
      if (res) {
        await DeleteConvenio(id)
          .then(() => {
            Vue.$toast.success(
              "El Convenio fue eliminado exitosamente.",
              {}
            );
          })
          .catch((response) => {
            console.log(response.Error);
            Vue.$toast.error("El Convenio no pudo eliminarse.");
          });
        await this.loadTable();
      } else {
        // mostrar error
