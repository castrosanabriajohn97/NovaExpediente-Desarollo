    ...
    >
      <template #header>
        ...
        <div>
          ...
          >
          <CBadge
            v-show="filters.publicas"
            v-for="item in filters.publicas"
            :key="item.nombre"
            style="background-color: #29abe2"
            class="mr-2"
            >{{ item }}</CBadge
      ...
      <template #empty> No se encontraron noticias. </template>
      <template #loading>
        Cargando la información de noticias. Por favor espere.
      </template>
      <Column
        field="titulo"
        header="Título"
      ...
      >
        <template #body="slotProps">
          ...
          <a
            ...
            @click.prevent="editNew(slotProps.data)"
            >{{ slotProps.data.titulo }}</a
          >
        </template>
      </Column>
      ...
      <Column header="Acciones" :sortable="false" headerStyle="width: 10%">
        <template #body="slotProps">
          ...
          >
            ...
            <CDropdownItem @click="viewNew(slotProps.data)">
              <CIcon name="cil-magnifying-glass" />&nbsp; Ver
            </CDropdownItem>
            <CDropdownItem @click="deleteNew(slotProps.data)">
...
import { News, Categories, Languages, DeleteNoticia } from "../request";
...
export default {
  ...
  methods: {
    async getNewsCategories() {
      ...
      this.newsCategories = noticiasCategorias.map(({ nombre }) => ({
        name: nombre,
        value: nombre,
      }));
    },
    async getLanguages() {
      ...
      this.newsLanguages = idiomas.map(({ nombre }) => ({
        name: nombre,
        value: nombre,
      }));
    },
    ...
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    viewNew(item, _x, _event) {
      this.selectedNew = item;
      this.$emit("view-new", this.selectedNew.idNoticia);
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    editNew(item, _x, _event) {
      this.selectedNew = item;
      this.$emit("edit-new", this.selectedNew.idNoticia);
    },
    createNew() {
      this.$emit("create-new");
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    async deleteNew(item, _x, _event) {
      ...
      if (res) {
        await DeleteNoticia(id)
          .then(() => {
            Vue.$toast.success("La noticia fue eliminada exitosamente.", {});
          })
          .catch((response) => {
            console.log(response.Error);
            Vue.$toast.error("La noticia no pudo eliminarse.");
          });
        await this.loadTable();
      } else {
        // mostrar error
