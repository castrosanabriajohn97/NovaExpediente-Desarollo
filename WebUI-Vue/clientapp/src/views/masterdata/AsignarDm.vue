  ...
  <div>
    ...
    <CCard style="border-radius: 16px !important">
      <CCardHeader
        ...
        "
      >
        <strong> Asignar un líder a un colaborador</strong>
          ...
          <CRow style="margin-bottom: 20px">
            <CCol sm="6">
              ...
              <CRow>
                ...
                <CCol sm="9" class="align-self-center">
                  <span>{{ this.solicitud.deliverymanager }}</span>
                </CCol>
              </CRow>
            </CCol>
              ...
              <CRow>
                ...
                <CCol sm="9" class="align-self-center">
                  <FormulateInput
                    ...
                    :options="colaboradoresDMList"
                    type="select"
                    label="Nuevo líder"
                    placeholder="Seleccione un líder"
                    validation="required|not:0"
                    validation-name="Líder"
                    :validation-messages="{
                      required: 'Seleccione un líder.',
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(() => this.cargarDMs(this.colaborador.idcolaborador), 2000);
  },
  methods: {
      ...
      try {
        ...
        return {
          ...
          solicitud: {
            ...
            iddeliverymanager: colaborador.iddeliverymanager,
            deliverymanager: colaborador.deliverymanager,
            iddeliverymanagerOld: colaborador.iddeliverymanagerOld,
          },
        };
      } catch (error) {
    ...
    async selected(item) {
      switch (this.TipoListaPick) {
        case "Colaboradores":
          ...
          // traer detalle de colaborador

          this.cargarDetalleColaborador(item.id, false);
          await this.cargarDMs(item.id);
          break;
    ...
    async cargarDetalleColaborador(id, fromJson) {
      this.cargandoDatos = true;
      const {
        data: { colaborador },
      ...
      if (
        colaborador.idDeliveryManager != null &&
        colaborador.idDeliveryManager != 0
      ) {
        //  this.solicitud.iddeliverymanager = colaborador.idDeliveryManager;

        if (
          this.datosSolicitud.estadoSolicitud != "AP" &&
          this.datosSolicitud.estadoSolicitud != "DE"
        ) {
          this.solicitud.deliverymanager = colaborador.deliveryManager;

          this.fotoLider = await this.GetPhotoById(
            colaborador.idDeliveryManager
          );
          this.solicitud.iddeliverymanagerOld = colaborador.idDeliveryManager;
        } else {
          this.fotoLider = await this.GetPhotoById(
            this.solicitud.iddeliverymanagerOld
          );
        }

        //   this.solicitud.deliverymanager = colaborador.deliveryManager;

        // this.fotoLider = await this.GetPhotoById(colaborador.idDeliveryManager);
      }

      this.fotoNuevoLider = await this.GetPhotoById(
        this.solicitud.iddeliverymanager
      );
    ...
    async cargarDMs(id) {
      ...
      } = await GetColaboradoresDMAsignar(id);
      this.colaboradoresDMList = colaboradoresDM.map(
      ...
      // this.colaboradoresDMList = this.colaboradoresDMList.filter(
      //   x => x.value != this.solicitud.iddeliverymanagerOld
      // );
    },
  ...
  watch: {
    /**
     * Controla el cambio de alguno de los campos de asignar dm, esto permitirá marcar
