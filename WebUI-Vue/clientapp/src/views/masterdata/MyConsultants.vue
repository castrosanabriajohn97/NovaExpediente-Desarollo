  ...
  <div class="wrapper vld-parent" ref="formContainer">
    ...
    <CCard style="border-radius: 16px !important; margin-top: 20px;">
      <CCardHeader style="border-radius: 16px !important; padding-bottom: 0px; border-bottom: transparent !important; background-color: #FFFFFF;">
        <strong>Mis consultores</strong>
      </CCardHeader>
      <CCardBody style="padding-top: 0px; padding-bottom: 0px;">       
        <p> Solicitudes de cambio de mis consultores a cargo.</p>
      </CCardBody>
    </CCard> 
...
export default {
  ...
  async mounted() {
    await this.cargarDatos("Consultants");
  },
  methods: {
    ...
    cargarSolicitud(solicitud) {
      this.$router.push({
        name: "MasterData",
        params: {
          origen: "Consultants", // origen desde donde se accedio la vista
    ...
    async cargarDatos(tipo) {
      let consultants = this;
      let loader = consultants.showLoader();

      consultants.loading = true;

      await consultants
        .getLista(this.$root.infoColaboradorActual.id, tipo)
        // lista de solicitudes
        .then(function(result) {
          consultants.solicitudes = result.data.solicitudesMasterData;
          consultants.loading = false;
          consultants.hideLoader(loader);
        })
        .catch(error => {
