...
<template>
  <div>
    ...
    <CCard style="border-radius: 16px !important;">
      ...
      >
        <strong>Asignar una empresa a un colaborador</strong>
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(
      () =>
        (this.empresasList = this.empresasList.filter(
          x => x.value != this.colaborador.idempresacorporacion
        )),
      1000
    );
  ...
  methods: {
    ...
    async cargarDetalleColaborador(id) {
      ...
      this.empresaNombre = colaborador.empresaCorporacion;
