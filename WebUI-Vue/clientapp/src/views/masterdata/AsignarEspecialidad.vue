...
<template>
  <div>
    ...
    <CCard style="border-radius: 16px !important;">
      ...
      >
        <strong> Asignar una especialidad a un colaborador</strong>
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(
      () =>
        (this.especialidadList = this.especialidadList.filter(
          x => x.label != this.colaborador.ramaespecialidad
        )),
      1000
    );
  ...
  methods: {
      ...
      try {
        ...
        return {
          ...
          solicitud: {
            ...
            ramaespecialidad: colaborador.ramaespecialidad
          }
        };
      } catch (error) {
    ...
    async onChangeEspecialidad() {
      this.solicitud.ramaespecialidad = this.getNombreEspecialidad(
        this.solicitud.ramaespecialidad
      );
    },

    getNombreEspecialidad(id) {
      const name = this.especialidades.filter(x => x.value == id)[0].label;
      return name;
    },
    ...
    selected(item) {
      switch (this.TipoListaPick) {
        case "Colaboradores":
          ...
          // ir al detalle para obtener la rama especialidad
    ...
    async cargarDetalleColaborador(id) {
      ...
      this.ramaespecialidad = colaborador.ramaespecialidad;
  ...
  watch: {
    /**
     * Controla el cambio de alguno de los campos de especialidad de colaborador, esto permitirá marcar
