...
<template>
  <div>
    ...
    <CCard style="border-radius: 16px !important;">
      ...
      >
        <strong> Asignar un puesto a un colaborador</strong>
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(
      () =>
        (this.puestoList = this.puestoList.filter(
          x => x.value != this.colaborador.idpuesto
        )),
      1000
    );
  ...
  methods: {
    ...
    async cargarDetalleColaborador(id) {
      ...
      this.puestoNombre = colaborador.puesto;
  ...
  watch: {
    /**
     * Controla el cambio de alguno de los campos de puesto de colaborador, esto permitirá marcar
