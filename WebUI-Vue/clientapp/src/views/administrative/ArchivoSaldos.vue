...
<!-- Plantilla de Carga de saldos -->
<template>
  <div>
    <CCard>
      <CCardHeader>
        <CIcon name="cil-justify-center" /><strong>
          Cargar archivo de saldos de vacaciones</strong
        >
      ...
      <CCardBody>
        ...
        >
          ...
          <CRow>
            <CCol sm="6">
              <label>Archivo de saldos de vacaciones</label>
              <FormulateInput
                type="myfileupload"
                name="archivo"
                ...
                class="hint"
                tipo-documento="Archivo de saldos de vacaciones"
                estado-solicitud="RE"
                :eliminada="false"
                :visor-padre="false"
      ...
      <CRow class="pt-2">
        <CCol sm="12" lg="12" class="pull-right">
          ...
          >
            ...
            <template #empty>
              No se encontraron cargas de saldos de vacaciones.
            </template>
            <template #loading>
              Cargando la información de cargas de saldos de vacaciones. Por favor
              espere.
            </template>
...
import { CargaVacaciones } from "./request";
...
export default {
  ...
  methods: {
    historial() {
      this.$router.push({ name: "Carga de saldos de vacaciones" });   
    },
    formatearFecha(date) {
...
console.log(countColumns,'countColumns');
      ...
      if (countColumns != 3) {
        Vue.$toast.error(
          "La cantidad de columnas del archivo es incorrecta, deben ser 3 columnas.",
          {}
        );
        return false;
      }
      ...
      if (!Object.keys(data).includes("SALDO")) {
        Vue.$toast.error("El archivo no contiene la columna SALDO.", {});
        return false;
      }
        ...
        if (!this.visorEnable) {
          ...
          if (resultadoValidacion.esValido) {
            const res = await this.Confirmacion_Generico(
              "Desea cargar el archivo de saldos de vacaciones",
              "Carga de saldos de vacaciones"
            );
            var result = "";
                  ...
                  await CargaVacaciones(
                    ...
                    .then(response => {
                      ...
                      Vue.$toast.success(
                        ...
                        this.$router.push({ path: "CargaSaldosVacaciones" }); 
                    })
                    .catch(error => {
