    ...
    <div>
      <CCard style="border-radius: 16px !important; margin-top: 20px">
        <CCardHeader
          ...
          "
        >
          <strong>Bitacora de Vacaciones</strong>
        </CCardHeader>
        <CCardBody style="padding-top: 0px; padding-bottom: 0px">
          <p>Lista de acciones de personal pendientes.</p>
        </CCardBody>
      </CCard>
          ...
          <CRow>
            <CCol lg="12">
              <DataTable
                ...
                :rows="10"
                dataKey="idbitacoravacaciones"
                :rowHover="true"
                :selection.sync="bitacorasSeleccionadas"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} bitacoras"
                :sortOrder="1"
                stateStorage="session"
                stateKey="dt-state-bitacoras"
                selectionMode="single"
                :autoLayout="true"
                @update:filters="filterApplied"
                @filter="filterApplied"
              >
                <template #empty> No se encontraron bitacoras. </template>
                <template #loading>
                  Cargando la información de bitacoras. Por favor espere.
                </template>
                <!--   <Column
                ...
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Id de nomina</span>
                    {{ slotProps.data.idnomina }}
                  </template>
                </Column>
                ...
                >
                  <template #body="slotProps">
                    ...
                    <a style="cursor: hand">{{ slotProps.data.fechainicio }}</a>
                  </template>
                </Column>
                <Column
                  header="Fecha de fin"
                ...
                >
                  <template #body="slotProps">
                    ...
                    <a style="cursor: hand">{{ slotProps.data.fechafin }}</a>
                  </template>
                </Column>
                ...
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Id de solicitud</span>
      ...
      <div>
        <CRow class="pt-2 mb-4">
          <CCol sm="12" lg="12" class="pull-right">
            <label
              >Desea exportar el listado de bitacoras?</label
            >
            <!-- <CInputRadioGroup
              class="col-sm-12"
...
import { GetBitacoraVacaciones } from "./request";

export default {
  name: "BitacoraVacaciones",
  data() {
    return {
      dt: null,
  ...
  methods: {
    ...
    async getLista() {
      try {
        const  {data}  = await GetBitacoraVacaciones();
      ...
      //this.$refs.dt.exportCSV(); //funciona el de la tabla pero no permite cambiar el nombre del file
          ...
          data = data.map(
            ({
              ...
              Id_Bitacora: idbitacoravacaciones,
              Id_Nomina: idnomina,
              Fecha_Inicio: fechainicio,
              Fecha_Fin: fechafin,
              Id_Solicitud: idsolicitudmasterdata,
              Mensaje: mensaje,
              Cant_Dias: cantDias,
              Estado: estado
            })
          ...
          XLSX.utils.book_append_sheet(wb, ws, "BitacoraVacaciones.csv");
          XLSX.writeFile(wb, `BitacoraVacaciones_${this.getDateTime()}.csv`);
