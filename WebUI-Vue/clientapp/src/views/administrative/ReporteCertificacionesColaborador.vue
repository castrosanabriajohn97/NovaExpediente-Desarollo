    ...
    <div>
      <CCard style="border-radius: 16px !important; margin-top: 20px">
        <CCardHeader
          ...
          "
        >
          <strong>Reporte de Certificaciones</strong>
        </CCardHeader>
        <CCardBody style="padding-top: 0px; padding-bottom: 0px">
          <p>Certificaciones por Colaborador.</p>
        </CCardBody>
      </CCard>
          ...
          <CRow>
            <CCol lg="12">
              <DataTable
                ...
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} certificaciones"
                :sortOrder="1"
                stateStorage="session"
                stateKey="dt-state-reporte-cert"
                selectionMode="single"
                :autoLayout="true"
                @update:filters="filterApplied"
                @filter="filterApplied"
              >
                <template #empty> No se encontraron certificaciones. </template>
                <template #loading>
                  Cargando la información de certificaciones. Por favor espere.
                </template>
                <!--   <Column
                        selectionMode="multiple"
                        headerStyle="width: 3em"
                      ></Column>-->
                <Column field="identificacion" header="Id" :sortable="true">
                ...
                <Column
                  ...
                  :sortable="true"
                  sortField="certificacion"
                  filterField="certificacion"
                  filterMatchMode="contains"
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Certificación</span>
                    {{ slotProps.data.certificacion }}
                  </template>
                </Column>
                <Column
                ...
                >
                  <template #body="slotProps">
                    <span class="p-column-title">Fecha certificación</span>
                    {{ slotProps.data.fechaCertificacion }}
                  </template>
                </Column>
                <Column
      ...
      <div>
        <CRow class="pt-2 mb-4">
          <CCol sm="12" lg="12" class="pull-right">
            <label>Desea exportar el reporte?</label>
            <!-- <CInputRadioGroup
                class="col-sm-12"
...
import { GetReporteCertificacionesColaborador } from "./request";

export default {
  name: "ReporteCertificaciones",
  data() {
    return {
      dt: null,
      lineasSeleccionadas: [],
  ...
  methods: {
    ...
    async getLista() {
      try {
        const { data } = await GetReporteCertificacionesColaborador();
    ...
    async exportCSV() {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "CertificacionesColaborador.csv");
      XLSX.writeFile(
        wb,
        `ReporteCertificacionesColaborador${this.getDateTime()}.csv`
      );
