        ...
        >
          <CRow>
            <CCol sm="1" class="align-self-center text-center">
              <img
                src="/img/icono-solicitud-permiso.png"
          ...
          <CRow>
            ...
            <CCol sm="4" class="align-self-center text-left">
              <label>Permiso con goce de salario?</label>
              <FormulateInput
                type="myInputSwitch"
                name="Goce"
                :input-class="['form-control', 'readonly']"
                validation-name="Goce de Salario?"
                class="hint"
                :disabled="!puedePintar"
              />
            </CCol>
            <CCol sm="4" class="align-self-center text-left">
              <FormulateInput
                ...
                :validation-messages="{
                  required: 'Seleccione un tipo de permiso.',
                  not: 'Debe de seleccionar una opción válida.',
                }"
                v-show="this.solicitud.Goce == 'Si'"
                :disabled="!puedePintar"
              />
            </CCol>
          </CRow>
          ...
          <CRow>
            <CCol sm="1" class="align-self-center text-center"> </CCol>
            <CCol sm="4">
              <label v-show="this.solicitud.Goce == 'Si'"
                >Archivo del permiso</label
              >
              <FormulateInput
                type="myfileupload"
                name="archivo"
                ...
                class="hint"
                tipo-documento="Archivo permiso"
                :estado-solicitud="datosSolicitud.estadoSolicitud"
                :eliminada="false"
                :visor-padre="false"
                :habilitar-controles="
          ...
          <CRow style="margin-bottom: 20px">
            ...
            <CCol sm="10">
              <FormulateInput
                ...
                label="Indique alguna descripción adicional sobre la solicitud de permiso requerida"
...
import { GetTipoPermisoGoce } from "./request";
...
export default {
  ...
  methods: {
    ...
    muestraBotonera() {
      if (
        this.origen == "PendientesAtender" ||
        this.origen == "CertificacionesTramitadas"
      ) {
        if (
          !this.idResponsable ||
          this.idResponsable == this.$root.infoColaboradorActual.id
        ) {
          return true;
        } else {
          return false;
    ...
    async cargarTiposPermiso() {
      const {
        data: { tiposPermisoGoce },
      } = await GetTipoPermisoGoce();
      this.tiposPermiso = tiposPermisoGoce.map(
        ({ idtipopermisogoce, nombre }) => ({
          label: nombre,
          value: idtipopermisogoce,
        })
      );
    },

    getTipoNombre(id) {
      if (this.solicitud.Goce == "No") {
        return "Sin Goce";
      } else {
        const name = this.tiposPermiso.filter((x) => x.value == id)[0].label;
        return name;
      }
    },
    ...
    obtenerObjetoSolicitudEnviar(accion) {
      this.solicitud.TipoNombre = this.getTipoNombre(this.solicitud.Tipo);
      return {
        idSolicitud: this.datosSolicitud.idSolicitudMasterData,
        idTipoSolicitud: this.datosSolicitud.idTipoSolicitud,
        codigoEstado: this.datosSolicitud.estadoSolicitud,
    ...
    async procesarAccion_Denegar(accion) {
      let opciones = {
        ...
        mensajeDialogo:
          "Registre una justificación para denegar la solicitud de permisos",
        btnOk: "Denegar",
        btnCancel: "Cancelar",
        mensajeExito: "Solicitud denegada.",
        mensajeError:
          "Se ha presentado un inconveniente al denegar la solicitud de permisos.",
      };
      await this.enviarSolicitud(accion, opciones);
    },
    async procesarAccion_Aprobar(accion) {
      let opciones = {
        ...
        titulo: "Solicitud administrativa",
        mensajeDialogo: "Está seguro de aprobar la solicitud de permisos?",
        btnOk: "Si",
        btnCancel: "No",
        mensajeExito: "Solicitud aprobada.",
        mensajeError:
          "Se ha presentado un inconveniente al aprobar la solicitud de permisos.",
      };
      await this.enviarSolicitud(accion, opciones);
    },
    ...
    async submitSolicitud(formValues) {
      try {
        if (this.solicitud.estado == common.SOLICITUD_SINMODIFICACIONES) {
          ...
          const validar = this.validaDatos();
    ...
    validaDatos() {
      if (this.solicitud.cantDias < 1) {
        Vue.$toast.error(
          "La solicitud debe contener al menos un día de permisos.",
          {}
        );
        return false;
      }

      if (this.solicitud.Goce == "Si") {
        if (
          this.solicitud.Tipo == 4 &&
          this.solicitud.descripcion.trim() == ""
        ) {
          Vue.$toast.error(
            "La solicitud de tipo caso especial requiere de una descripción.",
            {}
          );
          return false;
        }
      ...
      if (this.solicitud.Goce == "Si") {
        if (!this.solicitud.Tipo) {
          Vue.$toast.error("Debe de seleccionar un tipo de permiso.", {});
          return false;
        }
      }
