...
<template>
  <div>
    ...
    <DataTable
      ...
      dataKey="idUnoAUno"
      :rowHover="true"
      :selection.sync="selectedUnoaUNo"
      :filters.sync="filters"
      :loading="loading"
      paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
      :rowsPerPageOptions="[5, 10, 25, 50]"
      currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} uno a unos"
      sortField="idUnoAUno"
      :sortOrder="1"
    ...
    >
      <template #header>
       ...
       <!--    <CCard color="light">
                ...
                >
                  <span class="p-float-label">
                    <MultiSelect
                      ...
                      :filter="true"
                    />
                    <label for="dropdown">Categorías</label>
                  </span>
                </div>
              </CRow> 
            </CCardBody>
        ...
        <!-- <div>
          ...
          >
          <CBadge
            v-show="filters.categorias"
            v-for="item in filters.categorias"
            :key="item.nombre"
            style="background-color: #29abe2"
            class="mr-2"
            >{{ item }}</CBadge
      ...
      <template #loading>
        Cargando la información de los uno a uno. Por favor espere.
      </template>
      <Column
        field="anno"
        header="Año"
      ...
      >
        <template #body="slotProps">
          ...
          {{ slotProps.data.colaborador }}
        </template>
      </Column>

      <Column
        field="estadoColaborador"
        sortField="estadoColaborador"
        header="Estado General"
        filterField="estadoColaborador"
        filterMatchMode="contains"
        :sortable="true"
        headerStyle="width: 20%"
      >
        <template #body="slotProps">
          <span class="p-column-title">Estado General</span>
          {{ slotProps.data.estadoColaborador }}
        </template>
      </Column>
      ...
      <Column header="Acciones" :sortable="false" headerStyle="width: 10%">
        <template #body="slotProps">
          ...
          >
            ...
            <CDropdownItem @click="edit(slotProps.data)">
              <CIcon style="color: darkblue;" name="cil-pencil" />&nbsp;&nbsp; Editar
            </CDropdownItem>
            <!-- <CDropdownItem @click="view(slotProps.data)">
              <CIcon name="cil-magnifying-glass" />&nbsp; Ver
            </CDropdownItem> -->
            <!-- <CDropdownItem @click="deleteConvenio(slotProps.data)">
...
import { GetUnoaUnos } from "../request";
...
export default {
  ...
  methods: {
    ...
    async loadTable() {
      const {
        data: { unosAUnos }
      } = await GetUnoaUnos(this.$root.infoColaboradorActual.id);

      this.listado = unosAUnos.map(
        ({
      idUnoAUno,
      idColaborador,
    ...
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    view(item, _x, _event) {
      this.selectedUnoaUNo = item;
      this.$emit("view", this.selectedUnoaUNo.idUnoAUno);
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    edit(item, _x, _event) {
      this.selectedUnoaUNo = item;
      this.$emit("edit", this.selectedUnoaUNo.idUnoAUno);
    },
    create() {
      this.$emit("create");
    },
    /*eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }]*/
    // async deleteUnoaUno(item, _x, _event) {
    //   const id = item.idUnoAUno;
    //   const res = await this.Confirmacion_Generico(
    //     `¿Desea eliminar el uno a uno?`,
    //     "Eliminar uno a uno"
    //   );
    //   if (res) {
    ...
    //       .then(() => {
    //         Vue.$toast.success(
    //           "El uno a uno fue eliminado exitósamente.",
    //           {}
    //         );
    //       })
    //       .catch((response) => {
    //         console.log(response.Error);
    //         Vue.$toast.error("El uno a uno no pudo eliminarse.");
    //       });
    //     await this.loadTable();
    //   } else {
    //     // mostrar error
