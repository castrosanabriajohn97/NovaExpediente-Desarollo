  ...
  <div>
    ...
    <CCard style="border-radius: 16px !important; margin-top: 20px;">
      <CCardHeader style="border-radius: 16px !important;  padding-bottom: 0px; border-bottom: transparent !important; background-color: #FFFFFF;">
        <strong>Carga de saldos de vacaciones</strong>
      </CCardHeader>
           ...
           <CCardHeader style="border-radius: 16px !important; border-bottom: transparent !important; background-color: #FFFFFF;">
            ...
            <CCardBody>  -->
              <DataTable
                ...
                :selection.sync="saldoSeleccionado"
              >
                <template #header>
                ...
                <Column
                  field="idcargavacaciones"
                  header="Id"
                  sortField="idcargavacaciones"
                  filterField="idcargavacaciones"
                ...
                >
                  <template #body="slotProps">
                    ...
                    {{ slotProps.data.idcargavacaciones }}
                  </template>
                </Column>
...
<script>
import { GetCargaVacaciones } from "./request";
import moment from "moment";
import Vue from "vue";
moment.locale("es");

export default {
  name: "CargaVacaciones",
  data() {
    return {
      saldos: [],
  ...
  methods: {
    ...
    cargarArchivo() {
        this.$router.push({ path: "ArchivoSaldos" }); 
    },
    async cargarTabla() {
      const {
        data: { cargas }
      } = await GetCargaVacaciones();
      this.saldos = cargas.map(
        ({
          idcargavacaciones,
          nombrearchivo,
          cantidadregistros,
          ...
          idcargavacaciones: idcargavacaciones,
    ...
    verArchivo() {
      var lista;
      if (this.saldoSeleccionado) {
        lista = JSON.parse(this.saldoSeleccionado.contenidoarchivo);
        const $this = this;
