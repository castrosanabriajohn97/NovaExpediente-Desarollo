          ...
          <CCard>
            ...
            <CCardBody>
              <DataTable
                ...
                :rows="10"
                dataKey="idpermisotrabajo"
                :rowHover="true"
                :filters.sync="filters"
                :loading="loading"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25, 50]"
                currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} permisos"
                sortField="nombre"
                :sortOrder="1"
                stateKey="dt-state-permiso"
                :autoLayout="true"
              >
                <template #header>
                  <CRow>
                ...
                <template #loading>
                  Cargando la información de permisos. Por favor espere.
                </template>
                ...
                <Column
                  field="tipopermiso"
                  header="Tipo"
                  sortField="tipopermiso"
                  filterField="tipopermiso"
                  filterMatchMode="contains"
                  :sortable="true"
                  headerStyle="width:10%;"
                >
                ...
                <Column
                  field="tipopermisogoce"
                  header="Categoría"
                  sortField="tipopermisogoce"
                  filterField="tipopermisogoce"
                  filterMatchMode="contains"
                  :sortable="true"
                  headerStyle="width:10%;"
                >
                  <template #body="slotProps">
                    ...
                    {{ slotProps.data.tipopermisogoce }}
                  </template>
                </Column>
...
<script>
import { GetPermisosPorPerfil } from "./request";
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(
      () => (
        (this.loading = false),
        (this.permisos = this.permisos.filter(function(item) {
  ...
  methods: {
    ...
    async cargarTabla() {
      ...
      const {
        data: { permisos }
      } = await GetPermisosPorPerfil(datos);
      this.permisos = permisos.map(
        ({
          idpermisotrabajo,
          idcolaborador,
          fechainicio,
          ...
          idpermisotrabajo: idpermisotrabajo,
          ...
          tipopermiso: this.setTipoPermiso(tipopermiso),
          tipopermisogoce: tipopermisogoce,
    ...
    async exportar() {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Permisos.csv");
      XLSX.writeFile(wb, `Permisos_${this.getDateTime()}.csv`);
