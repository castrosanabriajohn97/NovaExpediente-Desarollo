...
<template>
      ...
      <div>
        <CCard style="border-radius: 16px !important; margin-top: 20px">
          <CCardHeader
            ...
            "
          >
            <strong>Reporte de Formación</strong>
          </CCardHeader>
          <CCardBody style="padding-top: 0px; padding-bottom: 0px">
            <p>Formación por Colaborador.</p>
          </CCardBody>
        </CCard>
            ...
            <CRow>
              <CCol lg="12">
                <DataTable
                  ...
                  currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} formaciones"
                  :sortOrder="1"
                  stateStorage="session"
                  stateKey="dt-state-reporte-formaciones"
                  selectionMode="single"
                  :autoLayout="true"
                  @update:filters="filterApplied"
                  @filter="filterApplied"
                >
                  <template #empty> No se encontraron formaciones. </template>
                  <template #loading>
                    Cargando la información de formaciones. Por favor espere.
                  </template>
                  <!--   <Column
                  ...
                  <Column
                    header="Centro académico"
                    :sortable="true"
                    sortField="centroAcademico"
                    filterField="centroAcademico"
                    filterMatchMode="contains"
                  >
                    <template #body="slotProps">
                      <span class="p-column-title">Centro académico</span>
                      {{ slotProps.data.centroAcademico }}
                    </template>
                  </Column>
                  <Column
                  ...
                  >
                    <template #body="slotProps">
                      <span class="p-column-title">Grado académico</span>
                  ...
                  >
                    <template #body="slotProps">
                      <span class="p-column-title">Fecha de obtención</span>
                      {{ slotProps.data.anio }}
                    </template>
                  </Column>
  ...
  import { GetReporteFormacionColaborador } from "./request";
  
  export default {
    name: "ReporteFormacion",
    data() {
      return {
        dt: null,
        lineasSeleccionadas: [],
    ...
    methods: {
      ...
      async getLista() {
        try {
          let  {data}  = await GetReporteFormacionColaborador();


    this.reporte = data.map(
        ({ ACTIVO, CORREO,IDENTIFICACION,NOMBRE,CENTROACADEMICO,FECHANE,CARRERA,GRADOACADEMICO,GENERO,ANIO}) => ({
          activo: ACTIVO,
          correo: CORREO,
      ...
      async exportCSV() {
        //this.$refs.dt.exportCSV(); //funciona el de la tabla pero no permite cambiar el nombre del file
            ...
            XLSX.utils.book_append_sheet(wb, ws, "FormacionColaborador.csv");
            XLSX.writeFile(wb, `ReporteFormacionColaborador${this.getDateTime()}.csv`);
