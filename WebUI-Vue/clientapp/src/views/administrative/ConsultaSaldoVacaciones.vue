          ...
          <CCard>
            <CCardHeader>
              <CIcon name="cil-justify-center" /><strong>
                Consulta de saldo de vacaciones</strong
              >
              <small> Lista de saldos por colaborador</small>
            </CCardHeader>
            <CCardBody>
              <DataTable
                ref="dt"
...
<script>
import { GetColaboradorVacacionesPorPerfil } from "./request";

import Vue from "vue";
import XLSX from "sheetjs-style";

export default {
  name: "ConsultaSaldoVacaciones",
  components: {},
  data() {
    return {
      saldos: [],
  ...
  methods: {
    ...
    async cargarTabla() {
      ...
      } = await GetColaboradorVacacionesPorPerfil(id);
