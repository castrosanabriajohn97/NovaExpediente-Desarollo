...
<template>
  <div>
    ...
    <CCard style="border-radius: 16px !important;">
      ...
      >
        <strong> Renovación DIMEX</strong>
        ...
        >
          <CRow style="margin-bottom: 20px;">
            <CCol sm="1" class="align-self-center text-center">
              <img
                src="/img/icono-renovacion-dimex.png"
            ...
            <CCol sm="11">
              <FormulateInput
                ...
                type="textarea"
                rows="3"
                label="Indique alguna descripción adicional sobre la renovación DIMEX requerida"
  ...
  methods: {
    muestraBotonera() {
      if (this.origen == "PendientesAtender" || this.origen == "CertificacionesTramitadas" ) {
        if (this.idResponsable == this.$root.infoColaboradorActual.id ) {
          return true;
        } else {
          return false;
        }
    ...
    async procesarAccion_Denegar(accion) {
      let opciones = {
        ...
        mensajeDialogo:
          "Registre una justificación para denegar la renovación DIMEX",
        btnOk: "Denegar",
        btnCancel: "Cancelar",
        mensajeExito: "Solicitud denegada.",
        mensajeError:
          "Se ha presentado un inconveniente al denegar la renovación DIMEX."
      };
      await this.enviarSolicitud(accion, opciones);
    },
    async procesarAccion_Aprobar(accion) {
      let opciones = {
        ...
        titulo: "Solicitud administrativa",
        mensajeDialogo: "Está seguro de aprobar la renovación DIMEX?",
        btnOk: "Si",
        btnCancel: "No",
        mensajeExito: "Solicitud aprobada.",
        mensajeError:
          "Se ha presentado un inconveniente al aprobar la renovación DIMEX."
      };
      await this.enviarSolicitud(accion, opciones);
    },
