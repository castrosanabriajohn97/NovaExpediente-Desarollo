  ...
  <div>
    ...
    >
      <CCardHeader
        ...
        "
      >
        <strong>Chat de consulta para Informes</strong>
      </CCardHeader>
      <CCardBody
...
import {
  GetListaConversacionesInforme,
  GetListaDetalleConversacionInforme,
  InsertConversacionMensajeInforme,
  InsertConversacionesInforme,
} from "../request";

import { GetPhoto } from "../../../containers/request";
import { error } from "jquery";

export default {
  name: "ConsultaInformesIA",
  mixins: [common],
  props: {
    origen: { type: String, default: "" },
    idTipoSolicitud: { type: Number, default: 0 },
    tipoSolicitud: { type: String, default: "" },
    idSolicitud: { type: Number, default: null },
  },
  components: {},
  data() {
    return {
      ...
      idIA: "IA",
      textmessages: {
        ROOMS_EMPTY: 'No hay informes disponibles',
        ROOM_EMPTY: 'No se ha seleccionado ningún informe',
        NEW_MESSAGES: 'Nuevos mensajes',
        MESSAGE_DELETED: 'Este mensaje ha sido eliminado',
        MESSAGES_EMPTY: 'No hay mensajes',
        CONVERSATION_STARTED: 'La consulta comenzó el:',
        TYPE_MESSAGE: 'Escribe tu consulta',
        SEARCH: 'Buscar informe',
        IS_ONLINE: '🤖 Asistente en línea',
  ...
  methods: {
      ...
      this.messages = [
        {
          _id: "7890",
          indexId: 12092,
          content: "Mensaje sobre el informe",
          senderId: "1234",
          username: "John Doe",
          avatar: "assets/imgs/doe.png",
          date: "13 noviembre",
    ...
    async sendMessage({ content, roomId, files, replyMessage}) {
      ...
      await InsertConversacionMensajeInforme({
    ...
    async refreshConversations(roomId) {
      await GetListaConversacionesInforme(this.$root.infoColaboradorActual.correo, 0)
        .then((response) => {
    ...
    async refreshMessages(roomId) {
      ...
      await GetListaDetalleConversacionInforme(roomId)
      .then((response) => {
        console.log('refreshMessages', response.data.messages);
      ...
      await InsertConversacionesInforme({
        userId: this.$root.infoColaboradorActual.correo,
      })
      .then((response) => {
        console.log('addRoom ', response.data.conversationId);
