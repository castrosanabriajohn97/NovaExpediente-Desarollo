...
{
    public class EnviaSolicitudMobileCommandValidator : AbstractValidator<EnviaSolicitudMobileCommand>
    {
        public EnviaSolicitudMobileCommandValidator()
        {
            ...
            RuleFor(x => x.tipoSolicitud).Must(x => new[] { "Vacaciones", "Permisos", "Incapacidades", "ConstanciaSalario" }.Contains(x)).WithMessage("El valor del tipo de la solicitud debe ser (Vacaciones/Permisos/Incapacidades/ConstanciaSalario).");
        }
    }
}
